...
def crop(src):
    ...
    if args.debug:
        ...
        cv.imshow("debug", img_blur)
        cv.waitKey(500)
        cv.imshow("debug", detected_edges)
        cv.waitKey(500)
...
def process(filename, destname, autoLevelOfs = 0, middleLevel = 1.0):
    src = cv.imread(filename)
    dest = cv.imread(destname)
    dest = crop(src)
    ...
    src_sm = cv.resize(src, (50, 50), interpolation=cv.INTER_CUBIC)
    src_hsv = destHSV = cv.cvtColor(src_sm, cv.COLOR_BGR2HSV)

    dest_sm = cv.resize(dest, (50, 50), interpolation=cv.INTER_CUBIC)
    dest_hsv = destHSV = cv.cvtColor(dest_sm, cv.COLOR_BGR2HSV)

    '''    
    color = ('b','g','r')
    for i, col in enumerate(color):
        histr = cv.calcHist([dest_hsv],[i],None,[256],[0,256])
        plt.plot(histr, color = col)
        plt.xlim([0,256])
        
        histr = cv.calcHist([src_hsv],[i],None,[256],[0,256])
        plt.plot(histr, color = "black")
        plt.xlim([0,256])
    plt.show()'''

    dh, ds, dv = cv.split(dest_hsv)
    sh, ss, sv = cv.split(src_hsv)
    fig = plt.figure()
    axis = fig.add_subplot(1, 1, 1, projection="3d")
    axis.scatter(dh.flatten(), ds.flatten(), dv.flatten(), facecolors="red", marker=".")
    axis.scatter(sh.flatten(), ss.flatten(), sv.flatten(), facecolors="blue", marker=".")
    axis.set_xlabel("Hue")
    ...
    '''
    lut = np.asarray([[[dh[i], ds[i], dv[i]] for i in range(256)]], dtype=np.uint8)    
    src_hsv = cv.LUT(src_hsv, lut)
    src_sm = cv.cvtColor(src_hsv, cv.COLOR_HSV2BGR)
    cv.namedWindow("image", cv.WINDOW_AUTOSIZE)
    cv.imshow("image", src_sm)
    cv.waitKey(500)
    '''
...
for folder in os.listdir(base):
    ...
    for filename in os.listdir(os.path.join(base, folder)):
        if filename.endswith(".JPG") and os.path.exists(os.path.join(dest, folder, filename)):
